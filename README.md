# All in One Accessibility®: Gridsome Plugin
...
```sh
npm install gridsome-plugin-all-in-one-accessibility
```

## Setup & Usage

Add it as a plugin in `gridsome.config.js`

```js
plugins: [
    {
      use: 'gridsome-plugin-all-in-one-accessibility',
      options: {
      }
    }
  ]
...
Open All in One Accessibility Settings page on Gridsome by using following URL:
```<gridsome website URL>/gridsome-plugin-all-in-one-accessibility```
...
- **[Gridsome All in One Accessibility](https://www.skynettechnologies.com/gridsome-website-accessibility)**
- **[How to install All in One Accessibility Extension on Gridsome blog](https://www.skynettechnologies.com/blog/gridsome-web-accessibility-widget-installation)**
